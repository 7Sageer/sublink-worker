...
要在本地运行此项目以进行开发或测试，请按以下步骤操作：

### 先决条件

*   您的系统上已安装 [Node.js](https://nodejs.org/) (包含 npm)。
*   已安装 Git 用于克隆代码仓库。
...
1.  **克隆代码仓库:**
    ...
    cd your-repo-name
    ```
    (请将 `your-username/your-repo-name` 替换为实际的仓库 URL)
...
3.  **运行本地开发服务器:**
    ...
    npm run dev
    ```
    此命令会使用 `wrangler dev` 启动一个本地服务器 (通常在 `http://localhost:8787`) 并模拟 Cloudflare 环境，包括 KV 存储。任何写入 KV 命名空间 (如 `SUBLINK_KV`) 的数据都将本地存储在项目中的 `.wrangler` 目录下。

### KV 命名空间说明

脚本 `npm run setup-kv` (或 `node scripts/setup-kv.js`) 用于管理*已部署*的 Cloudflare Workers 环境中的 KV 命名空间。它会与您的 Cloudflare 帐户交互，以创建或查找必要的 KV 命名空间，并相应地更新 `wrangler.toml` 文件。

**对于使用 `npm run dev` 进行本地开发，您不需要运行 `npm run setup-kv`**，因为 `wrangler dev` 会自动处理本地 KV 模拟。
